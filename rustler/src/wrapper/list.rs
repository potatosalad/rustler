...
pub unsafe fn get_list_cell(env: NIF_ENV, list: NIF_TERM) -> Option<(NIF_TERM, NIF_TERM)> {
    ...
    let success = crate::sys::enif_get_list_cell(env, list, head.as_mut_ptr(), tail.as_mut_ptr());
...
pub unsafe fn get_list_length(env: NIF_ENV, list: NIF_TERM) -> Option<usize> {
    let mut len: u32 = 0;
    let success = crate::sys::enif_get_list_length(env, list, &mut len);
