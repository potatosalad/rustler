...
pub unsafe fn make_existing_atom(env: NIF_ENV, name: &[u8]) -> Option<NIF_TERM> {
    ...
    let success = crate::sys::enif_make_existing_atom_len(
        env,
        name.as_ptr(),
        name.len(),
...
pub unsafe fn get_atom(env: NIF_ENV, term: NIF_TERM) -> Result<String, Error> {
    ...
    let mut len = 0;
    let success = crate::sys::enif_get_atom_length(env, term, &mut len, ERL_NIF_LATIN1);
    if success == 0 {
    ...
    let nbytes = crate::sys::enif_get_atom(env, term, bytes.as_mut_ptr(), len + 1, ERL_NIF_LATIN1);
