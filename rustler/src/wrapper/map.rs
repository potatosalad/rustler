...
pub unsafe fn get_map_value(env: NIF_ENV, map: NIF_TERM, key: NIF_TERM) -> Option<NIF_TERM> {
    ...
    let success = crate::sys::enif_get_map_value(env, map, key, result.as_mut_ptr());
...
pub unsafe fn get_map_size(env: NIF_ENV, map: NIF_TERM) -> Option<usize> {
    ...
    let success = crate::sys::enif_get_map_size(env, map, size.as_mut_ptr());
...
pub unsafe fn map_new(env: NIF_ENV) -> NIF_TERM {
    crate::sys::enif_make_new_map(env)
}
...
pub unsafe fn map_put(
    ...
    let mut result = MaybeUninit::uninit();
    let success = crate::sys::enif_make_map_put(env, map, key, value, result.as_mut_ptr());
...
pub unsafe fn map_remove(env: NIF_ENV, map: NIF_TERM, key: NIF_TERM) -> Option<NIF_TERM> {
    let mut result = MaybeUninit::uninit();
    let success = crate::sys::enif_make_map_remove(env, map, key, result.as_mut_ptr());
...
pub unsafe fn map_update(
    ...
    let mut result = MaybeUninit::uninit();
    let success = crate::sys::enif_make_map_update(env, map, key, new_value, result.as_mut_ptr());
...
pub unsafe fn map_iterator_create(env: NIF_ENV, map: NIF_TERM) -> Option<ErlNifMapIterator> {
    ...
    let success = crate::sys::enif_map_iterator_create(
        env,
        map,
        iter.as_mut_ptr(),
...
pub unsafe fn map_iterator_destroy(env: NIF_ENV, iter: &mut ErlNifMapIterator) {
    crate::sys::enif_map_iterator_destroy(env, iter);
}
...
pub unsafe fn map_iterator_get_pair(
    ...
    if crate::sys::enif_map_iterator_get_pair(env, iter, key.as_mut_ptr(), value.as_mut_ptr()) == 0
    {
        None
    } else {
...
pub unsafe fn map_iterator_next(env: NIF_ENV, iter: &mut ErlNifMapIterator) {
    crate::sys::enif_map_iterator_next(env, iter);
}
...
pub unsafe fn make_map_from_arrays(
    ...
    if crate::sys::enif_make_map_from_arrays(
        env,
        keys.as_ptr(),
        values.as_ptr(),
        keys.len() as usize,
