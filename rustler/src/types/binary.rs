...
impl Drop for OwnedBinary {
    fn drop(&mut self) {
        unsafe { crate::sys::enif_release_binary(&mut self.0) };
    }
}
...
impl<'a> Binary<'a> {
    ...
    pub fn from_owned(owned: OwnedBinary, env: Env<'a>) -> Self {
        ...
        let term = unsafe {
            Term::new(
                env,
                crate::sys::enif_make_binary(env.as_c_arg(), &mut owned.0),
            )
        };
        Binary {
            inner: owned.0,
    ...
    pub fn from_iolist(term: Term<'a>) -> Result<Self, Error> {
        let mut binary = MaybeUninit::uninit();
        if unsafe {
            crate::sys::enif_inspect_iolist_as_binary(
                term.get_env().as_c_arg(),
                term.as_c_arg(),
                binary.as_mut_ptr(),
            )
    ...
    pub fn make_subbinary(&self, offset: usize, length: usize) -> NifResult<Binary<'a>> {
        ...
        let raw_term = unsafe {
            crate::sys::enif_make_sub_binary(
